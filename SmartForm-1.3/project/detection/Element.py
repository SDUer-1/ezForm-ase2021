...
class Element:
    ...
    *************************
    '''
    def is_line(self, configuration):
        if self.height <= configuration.is_line_max_thickness and self.width >= configuration.is_line_min_length:
            return True
        return False

    def is_rectangle_or_square(self, configuration):
        '''
            ...
            if k != k_pre:
                ...
                if len(side) < 4:
                    ...
                    if len(sides) > 0 and k == slopes[-1] \
                            and not pop_pre and gap_to_pre < 2:
                        side = sides.pop()
                        side += noises
                        side.append(p)
                        k = slopes.pop()
    ...
    *******************************
    ...
    def pos_relation(self, element, pos_bias, board=None):
        '''
        ...
        '''
        if self.area != 0 and element.area != 0:
            ioa, iob = self.calc_intersection(element, pos_bias, board)
        else:
            return 3
    ...
    *************************
    '''
    def is_in_same_character_box(self, ele, configuration):
        if self.type not in ('rectangle', 'square') or ele.type not in ('rectangle', 'square'):
            return False
        configuration.set_same_character_box_parameter(self,ele)
        if max(self.character_area, ele.character_area) / min(self.character_area, ele.character_area) < 1.2 and \
                (self.is_on_same_line(ele, direction='h', bias_justify=configuration.same_character_box_updownside_bias,bias_gap=configuration.same_character_box_gap) or self.pos_relation(ele,configuration.same_character_box_pos_relation_bias) != 0):
            return True
        return False
    ...
    def character_box_merge_ele(self, ele):
        ...
        self.character_num = self.character_num + ele.character_num
    ...
    ********************
    '''
    def is_textbox_or_border(self, ratio):
        '''
        ...
        if len(self.contains) == 0 or self.is_character_box:
            return False
        # remove noise
        if self.type == 'square' and len(self.contains) == 1 and self.containment_area / self.area > ratio:
    ...
    def textbox_merge_and_extract_texts_content(self, alignment_bias=0, v_max_merged_gap=10):
        '''
        ...
        while changed:
            ...
            for text_a in texts:
                ...
                for text_b in temp_set:
                    if text_a.is_in_alignment(text_b, direction='v', bias=alignment_bias) and \
