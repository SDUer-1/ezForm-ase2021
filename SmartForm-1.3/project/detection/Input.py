...
class Input(Element):
    def __init__(self, guide_text, input_field, input_type=0, is_checkbox=False, is_embedded=False, placeholder=None):
        ...
        self.input_type = input_type              # 0:not classified, 1: I1(1), 2: I1(2), 3: I2, 4: I3, 5: I4
    ...
    def is_connected_field(self, bar, direction='h', bias_gap = 4, bias_justify = 4):
        '''
        Check if the bar is part of the input field
        '''
        for f in self.input_fields:
            if f.unit_group_id == bar.unit_group_id:
                if direction == 'v' and f.is_on_same_line(bar, direction='v', bias_gap=bias_gap, bias_justify=bias_justify):
                    return True
                if direction == 'h' and f.is_on_same_line(bar, direction='h', bias_gap=bias_gap, bias_justify=bias_justify):
                    return True
        return False
