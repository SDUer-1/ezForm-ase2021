...
class Table:
    ...
    def merge_vertical_texts_in_cell(self,row_align_bias):
        for row in self.rows:
            row.merge_vertical_texts_in_cell(row_align_bias)
    ...
    def insert_element(self, element, insert_bias = 1):
        '''
        ...
        '''
        for row in self.rows:
            if element.is_in_alignment(row, direction='h', bias=insert_bias):
                row.add_element(element)
                self.sort_rows()
                self.init_bound()
                return

    def split_columns(self,configuration):
        '''
        ...
        if self.heading is not None:
            for head in self.heading.elements:
                col = [head]
                configuration.set_split_col_max_bias_justify(head)
                for row in self.rows[1:]:
                    for ele in row.elements:
                        if not ele.is_module_part:
                            if head.is_justified(ele, direction='v', max_bias_justify=configuration.split_col_max_bias_justify) or\
                                    (head.location['left'] - configuration.split_col_max_bias_justify / 2 < ele.location['left'] and head.location['right'] + configuration.split_col_max_bias_justify / 2> ele.location['right']) or\
                                    (ele.location['left'] - configuration.split_col_max_bias_justify / 2 < head.location['left'] and ele.location['right'] + configuration.split_col_max_bias_justify / 2> head.location['right']):
