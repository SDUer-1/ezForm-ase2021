...
class Image:
    def __init__(self, img_file_name, resize_height=None, configuration=None):
        self.img_file_name = img_file_name
        self.img = cv2.imread(img_file_name)
        self.resize_height = resize_height
        self.resize_img_by_height()

        #self.blured_img = cv2.blur(self.img, (3, 3))
        #self.grey_img = cv2.cvtColor(self.blured_img, cv2.COLOR_BGR2GRAY)
    ...
    ************************
    ...
    def get_binary_map(self, min_grad=10):
        '''
        :param min_grad: if a pixel is bigger than this, then count it as foreground (255)
        :return: binary map
        '''
        ret, bin = cv2.threshold(self.gradient_map, min_grad, 255, cv2.THRESH_BINARY)
    ...
    def get_binary_map_for_scan(self, min_grad=40):
        '''
        :param min_grad: if a pixel is bigger than this, then count it as foreground (255)
        :return: binary map
        '''
        ...
        #kernel_2 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 1))
        # 开闭运算，先开运算去除背景噪声，再继续闭运算填充目标内的孔洞
        closed = cv2.morphologyEx(bin, cv2.MORPH_CLOSE, kernel_1)
        #closed = cv2.morphologyEx(closed, cv2.MORPH_CLOSE, kernel_2)
        #morph = cv2.morphologyEx(bin, cv2.MORPH_CLOSE, (150, 1))  # remove noises
        self.binary_map = closed
        cv2.namedWindow('binary',cv2.WINDOW_NORMAL)
        cv2.imshow('binary', self.binary_map)
        cv2.waitKey(0)
        return bin
    ...
    def get_binary_map_canny(self):
        self.binary_map = cv2.Canny(self.grey_img, 20, 250)
        cv2.namedWindow('binary', cv2.WINDOW_NORMAL)
        cv2.imshow('binary', self.binary_map)
        cv2.waitKey(0)
    ...
    ***************************
    ...
    def get_elements(self, min_area=20):
        ...
        '''
        self.all_elements = []
        contours, hierarchy = cv2.findContours(self.binary_map, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        for cnt in contours:
    ...
    def detect_rectangle_and_square_elements(self, configuration, hollow=True):
        if len(self.all_elements) == 0:
            self.get_elements()
        self.rectangle_elements = []
        self.square_elements = []
        for ele in self.all_elements:
            rect_squ_check = ele.is_rectangle_or_square(configuration)
            if rect_squ_check:
                if hollow:
    ...
    def detect_line_elements(self, configuration):
        if len(self.all_elements) == 0:
            self.get_elements()
        self.line_elements = []
        for ele in self.all_elements:
            if ele.is_line(configuration):
                ele.type = 'line'
    ...
    ***********************
    ...
    def visualize_elements_contours_individual(self, element_opt='all', board_opt='org',
        ...
        for ele in eles:
            if ele.is_rectangle_or_square(self.config):
